  ...
  {%- if sr.slug and sr.slug != '' and sr.url != site.url -%}
    function {{ region | remove: "-" }}Random() {
      $.getJSON("{{ sr.url }}/region/{{ sr.slug }}/data/all.json", function(data) {
        ...
        function reAdsTel() { return '&nbsp;<div class="btn-group"><button type="button" class="btn btn-link px-1 rounded-0 border-0 p-0" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">&phone;</button><div class="dropdown-menu px-2 dropdown-menu-right"><span class="text-nowrap">' + data[i].seller + '</span><br><a href="tel:+' + data[i].phone + '">+' + data[i].phone.substr(0, 2) + '&nbsp;' + data[i].phone.substr(2, 3) + '&nbsp;' + data[i].phone.substr(5, 3) + '&nbsp;' + data[i].phone.substr(8, 2) + '&nbsp;' + data[i].phone.substr(10, 2) + '</a><i class="d-none">' + data[i].id + '</i></div></div>&nbsp;'; }
        function reAdsType() {
          if (count > 0) {
            if (data[i].type.includes('{{ site.data.lang-uk.re_apartment }}') && data[i].rent == '' && data[i].price !== '') {
              div.append('<div class="card mx-2"><div class="card-body"><strong>{{ site.data.lang-uk.re_sell_apartment }}</strong> {{ site.data.lang-uk.re_surface_total }} ' + data[i].surface + '&nbsp;{{ site.data.lang-uk.m }}, {{ site.data.lang-uk.re_rooms }} ' + data[i].rooms + ', {{ site.data.lang-uk.re_na }} ' + data[i].floor + '{{ site.data.lang-uk.re_mu }} {{ site.data.lang-uk.re_floorci }} {{ site.data.lang-uk.re_address }} ' + data[i].address + '' + reAdsLocation() + '' + reAdsRegion() + ', ' + reAdsPrice() + ', ' + reAdsTel() + '</div></div>');
            ...
            } else if (data[i].rent == 1 && data[i].price !== '' && data[i].price_sqmt == '' && data[i].type.includes('{{ site.data.lang-uk.re_store }}')) {
              div.append('' + reAdsRent() + '');
