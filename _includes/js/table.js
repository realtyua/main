...
$(document).on('click', 'a.lightbox', function(event){
  ...
  options = {
    addCaptionHTMLFn: function(item[index], captionEl, isFake) {
        if(!item[index].title) {
            captionEl.children[0].innerHTML = '';
            return false;
        }
        captionEl.children[0].innerHTML = '<div class="pswp__caption__title">' + item[index].title + '</div>';
        if(item[index].caption) {
            captionEl.children[0].innerHTML += '<div class="pswp__caption__subtitle">' + item[index].caption + '</div>';
        }
        return true;
    },
  ...
  if (value !== '') {
    if (row.type.indexOf('{{ site.data.lang-uk.re_land }}') !== -1 || row.type.indexOf('{{ site.data.lang-uk.re_land | downcase }}') !== -1) {
      ...
      if (row.surface_land && row.surface_land !== '') {
        html.push('{{ site.data.lang-uk.re_surface }} <strong>' + row.surface_land + ' {{ site.data.lang-uk.re_m }}</strong><sup>2</sup>, '),
